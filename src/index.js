...
io.on('connection', (socket) => {
    ...
    socket.on('sendLocation', (croods, callback) => {
        const user = getUser(socket.id)
        io.to(user.room).emit('locationMessage', generateLocation(user.username, `https://www.google.com/maps/@${croods.latitude},${croods.longitude}`))
        callback()
    })
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('welcomeMessage', generateMessage('Admin', `${user.username} has left the chat!`))
            io.to(user.room).emit('roomData', {
                room:user.room,
                users: getUsersInRoom(user.room)
            })
